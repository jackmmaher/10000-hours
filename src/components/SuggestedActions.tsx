...
export function SuggestedActions({ actions }: SuggestedActionsProps) {
  const { setView, navigateToExploreWithFilter, navigateToJourneyTab, navigateToJourneyPlanning } =
    useNavigationStore()
  ...
  return (
    <div className="mb-10">
      <p className="font-serif text-sm text-ink/50 tracking-wide mb-5">You Might Consider</p>
    ...
    <>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm text-ink leading-snug">{action.message}</p>
          {action.detail && <p className="text-xs text-ink/40 mt-1">{action.detail}</p>}
        </div>
        {hasAction && (
          <svg
...
function ActionIcon({ type }: { type: SuggestedAction['type'] }) {
  ...
  switch (type) {
    ...
    default:
      return <div className="w-1.5 h-1.5 rounded-full bg-ink/30 mt-1.5 ml-1.5 mr-1.5" />
  }
}
